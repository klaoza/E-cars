...
// Fonction pour lire une date depuis l'entrée standard avec gestion des erreurs
...
// Structure pour représenter un client blacklisté
struct Blacklist {
    char cin_client[20];
    char raison[255];
...
void ajouterVoiture() {
    ...
    // Créer une nouvelle voiture et l'ajouter au parc
...
void ajouterVoitureAuParc(struct voiture* nouvelleVoiture) {
    // Ajouter la nouvelle voiture au début de la liste
...
int estVoitureDisponible(struct voiture* voiture, char date_debut[], char date_fin[]) {
    ...
    // La voiture est disponible pour la période spécifiée
    return 1; // Disponible
}
...
void louerVoiture() {
    ...
    while (voitureActuelle != NULL) {
        if (strcmp(voitureActuelle->matricule,mat) == 0) {
            // La voiture a été trouvée dans le parc

            // Demander le numéro de CIN du client
            ...
            // Ajouter la location à l'historique
...
void afficherHistoriqueTousMois(struct location* listeLocations) {
    ...
    for (int mois = 1; mois <= 12; mois++) {
        ...
        while (locationActuelle != NULL) {
            ...
            // Vérifier si la location a eu lieu pendant le mois spécifié
